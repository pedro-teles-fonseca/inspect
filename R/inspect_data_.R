...
#' @param warning_nas Logical value. If `TRUE` then the presence of `NA` or `NaN` values in `data` generates a warning message. `NA` and `NaN` pass silently otherwise (if `allow_nas` is set to`TRUE`).
#'
...
#'   * `data` is eligible to have been generated by a series of Bernoulli trials and has no `NA` or `NaN` values in `data`.
...
#'   * `x` is eligible to have been generated by a series of Bernoulli trials and `success` is not observed in `data`.
#'   * `x` is eligible to have been generated by a series of Bernoulli trials, there are `NA` or `NaN` values in `x` and both `allow_nas` and `warning_nas` are set to `TRUE`.
...
#' * An informative warning message is thrown if:
#'   * `x` contains valid categorical data that is eligible to be used as dichotomous data and `success` is not observed in `data`.
#'   * `x` contains valid categorical data that is eligible to be used as dichotomous data, there are `NA` or `NaN` values in `x` and both `allow_nas` and `warning_nas` are set to `TRUE`.
#' * An informative error message is thrown and the execution is stopped if:
